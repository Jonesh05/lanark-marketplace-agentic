...
export function ProductCard({ product }: { product: Product }) {
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-xl border border-border/60 bg-card transition hover:border-accent/40">
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-muted">
        <Link href={`/marketplace/${product.id}`} className="absolute inset-0 block">
          {product.image_url ? (
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-4">
        <div className="flex items-start justify-between gap-2">
          <div className="flex-1">
            <Link href={`/marketplace/${product.id}`} className="block">
              <h3 className="line-clamp-1 text-sm font-medium text-balance">{product.title}</h3>
              <p className="line-clamp-2 text-xs leading-relaxed text-muted-foreground">
                {product.description || copy.fallback.noDescription}
              </p>
            </Link>
          </div>
        ...
        <div className="mt-2 flex items-center justify-between gap-2 text-[10px] uppercase tracking-widest text-muted-foreground">
          <span className="line-clamp-1">
            {product.category ?? (product.stock > 0 ? `${product.stock} in stock` : copy.productCard.soldOut)}
            {product.brand ? ` · ${product.brand}` : ""}
          </span>
          {typeof product.rating === "number" && (
            <span className="shrink-0 font-mono text-accent">{product.rating.toFixed(1)}★</span>
          )}
        </div>
      </div>
    </article>
