...
export async function POST(req: Request) {
  ...
  const system = [
    ...
    "Listings are priced in their native currency and SETTLE in cUSD on-chain, but the user-facing symbol is USDm.",
    ...
    "BALANCE: getCusdBalance reads on-chain cUSD. When communicating the result to the user, always present the amount as USDm.",
    ...
    "CURRENCY: cUSD is the on-chain dollar stablecoin (1 USD ≈ 1 cUSD). For user-facing communication always quote prices and balances in USDm, not cUSD. Colombian pesos (COP) are NOT dollars: convert COP to cUSD by dividing by ~4000 (e.g. 8,500,000 COP ≈ 2,125 cUSD ≈ 2,125 USDm). NEVER treat a COP amount as if it were USDm. If the user gives an amount in COP, convert it to USDm before calling placeOffer, and tell them the converted USDm amount.",
